...
**Convention: don't update Closed submissions.** That they *can* be updated is a technical fact, not an invitation — Closed marks the record as done, and mutating it afterward undermines anything that treats closure as final (reporting, audit expectations, downstream integrations). Updating **Draft** and **Submitted** submissions is normal and fine. If a Closed record genuinely needs correction, treat it as a deliberate, admin-level data fix, not a routine write path.

If your application needs "Closed = immutable" semantics enforced (not just followed by convention), you enforce it yourself. See **`concepts/architectural-patterns` → "Closure Is Not a Write Lock"** for the canonical write-up of the design options (security policies, workflow filters, audit-trail kapps).
